...
pub struct CliArgs {
    ...
    /// Require a cryptographically verified, exact-event Buzz reply before a
    /// successful ACP channel turn is acknowledged. Default-off so existing
    /// deployments retain their current behavior unless explicitly enabled.
...
impl Config {
    ...
    pub fn from_args(mut args: CliArgs) -> Result<Self, ConfigError> {
        ...
        let allowed_respond_to = if let Some(raw) = args.allowed_respond_to {
            // Parse and canonicalize aliases before comparing. This keeps a
            // legacy allowed value such as `members` aligned with the same
            // RespondTo variant selected by clap.
